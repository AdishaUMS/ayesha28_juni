...
DXL_XL320_MODEL_ADDR        = 0
DXL_XL320_MODEL_SIZE        = 2
DXL_XL320_TORQUE_ADDR       = 24
DXL_XL320_TORQUE_SIZE       = 1
DXL_XL320_GOAL_POS_ADDR     = 30
DXL_XL320_GOAL_POS_SIZE     = 2
DXL_XL320_PRES_POS_ADDR     = 37
DXL_XL320_PRES_POS_SIZE     = 2



class DxlXl320SyncRWTestNode(Node):
    
    def __init__(self) -> None:
        super().__init__('DxlXl320SyncRWTestNode')
        self.get_logger().info('-----------[ DXL XL320 Sync RW Test ]-----------')
        ...
        self.sync_w_torque = dxl.GroupSyncWrite(
            self.porthandler,
            self.packethandler,
            DXL_XL320_TORQUE_ADDR,
            DXL_XL320_TORQUE_SIZE
        )

        self.sync_w_pos = dxl.GroupSyncWrite(
            self.porthandler,
            self.packethandler,
            DXL_XL320_GOAL_POS_ADDR,
            DXL_XL320_GOAL_POS_SIZE
        )

        self.sync_r_pos = dxl.GroupSyncRead(
            self.porthandler,
            self.packethandler,
            DXL_XL320_PRES_POS_ADDR,
            DXL_XL320_PRES_POS_SIZE
        )
    ...
    def dxlSearch(self) -> None:
        for i in range(self.joint_num_param):
            if self.joint_servo_param[i] == 'XL320':
    ...
    def readPos(self) -> None:
        ...
        for id in self.joint_id:
            dxl_res = self.sync_r_pos.isAvailable(id, DXL_XL320_PRES_POS_ADDR, DXL_XL320_PRES_POS_SIZE)
            ...
            pres_pos.append(
                self.sync_r_pos.getData(id, DXL_XL320_PRES_POS_ADDR, DXL_XL320_PRES_POS_SIZE)
            )
...
class DxlXl320RWTestNode(Node):

    def __init__(self) -> None:
        super().__init__('DxlXl320RWTestNode')
        self.get_logger().info('-----------[ DXL XL320 RW Test ]-----------')
    ...
    def enableTorque(self) -> None:
        for id in self.joint_id:
            dxl_res, dxl_err = self.packethandler.write1ByteTxRx(self.porthandler, id, DXL_XL320_TORQUE_ADDR, 1)
    ...
    def disableTorque(self) -> None:
        for id in self.joint_id:
            dxl_res, dxl_err = self.packethandler.write1ByteTxRx(self.porthandler, id, DXL_XL320_TORQUE_ADDR, 0)
    ...
    def readPos(self) -> None:
        ...
        for id in self.joint_id:
            pres_pos, dxl_res, dxl_err = self.packethandler.read2ByteTxRx(self.porthandler, id, DXL_XL320_PRES_POS_ADDR)
    ...
    def writePos(self) -> None:
        ...
        for id in self.joint_id:
            ...
            if id == self.joint_id[self.current_idx]:
                dxl_res, dxl_err = self.packethandler.write2ByteTxRx(self.porthandler, id, DXL_XL320_GOAL_POS_ADDR, self.current_amp)

            else:
                dxl_res, dxl_err = self.packethandler.write2ByteTxRx(self.porthandler, id, DXL_XL320_GOAL_POS_ADDR, 0)
