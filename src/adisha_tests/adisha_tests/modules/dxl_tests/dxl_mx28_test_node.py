...
DXL_MX28_MODEL_ADDR     = 0
DXL_MX28_MODEL_SIZE     = 2
DXL_MX28_TORQUE_ADDR    = 24
DXL_MX28_TORQUE_SIZE    = 1
DXL_MX28_GOAL_POS_ADDR  = 30
DXL_MX28_GOAL_POS_SIZE  = 2
DXL_MX28_PRES_POS_ADDR  = 36
DXL_MX28_PRES_POS_SIZE  = 2



class DxlMx28SyncRWTestNode(Node):

    def __init__(self) -> None:
        super().__init__('DxlMx28SyncRWTestNode')
        self.get_logger().info('-----------[ DXL MX28 Sync RW Test ]-----------')
        ...
        self.sync_w_torque = dxl.GroupSyncWrite(
            self.porthandler,
            self.packethandler,
            DXL_MX28_TORQUE_ADDR,
            DXL_MX28_TORQUE_SIZE
        )

        self.sync_w_pos = dxl.GroupSyncWrite(
            self.porthandler,
            self.packethandler,
            DXL_MX28_GOAL_POS_ADDR,
            DXL_MX28_GOAL_POS_SIZE
        )
    ...
    def dxlSearch(self) -> None:
        for i in range(self.joint_num_param):
            if self.joint_servo_param[i] == 'MX28':
                self.joint_id.append(self.joint_id_param[i])

        for id in self.joint_id:
            model_number, dxl_res, dxl_err = self.packethandler.read2ByteTxRx(self.porthandler, id, DXL_MX28_MODEL_ADDR)
            ...
            if dxl_res != dxl.COMM_SUCCESS:
                ...
                dxl_res = self.sync_r_pos.addParam(id, DXL_MX28_PRES_POS_ADDR, DXL_MX28_PRES_POS_SIZE)
    ...
    def readPos(self) -> None:
        ...
        for id in self.joint_id:
            dxl_res = self.sync_r_pos.isAvailable(id, DXL_MX28_PRES_POS_ADDR, DXL_MX28_PRES_POS_SIZE)
            ...
            pres_pos.append(
                self.sync_r_pos.getData(id, DXL_MX28_PRES_POS_ADDR, DXL_MX28_PRES_POS_SIZE)
            )
...
class DxlMx28RWTestNode(Node):

    def __init__(self) -> None:
        super().__init__('DxlMx28RWTestNode')
        self.get_logger().info('-----------[ DXL MX28 RW Test ]-----------')
    ...
    def enableTorque(self) -> None:
        for id in self.joint_id:
            dxl_res, dxl_err = self.packethandler.write1ByteTxRx(self.porthandler, id, DXL_MX28_TORQUE_ADDR, 1)
    ...
    def disableTorque(self) -> None:
        for id in self.joint_id:
            dxl_res, dxl_err = self.packethandler.write1ByteTxRx(self.porthandler, id, DXL_MX28_TORQUE_ADDR, 0)
    ...
    def readPos(self) -> None:
        ...
        for id in self.joint_id:
            pres_pos, dxl_res, dxl_err = self.packethandler.read2ByteTxRx(self.porthandler, id, DXL_MX28_PRES_POS_ADDR)
    ...
    def writePos(self) -> None:
        ...
        for id in self.joint_id:
            ...
            if id == self.joint_id[self.current_idx]:
                dxl_res, dxl_err = self.packethandler.write2ByteTxRx(self.porthandler, id, DXL_MX28_GOAL_POS_ADDR, self.current_amp)

            else:
                dxl_res, dxl_err = self.packethandler.write2ByteTxRx(self.porthandler, id, DXL_MX28_GOAL_POS_ADDR, 0)
